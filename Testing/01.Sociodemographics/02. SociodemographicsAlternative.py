...
bu_filter = (F.col('site_bu').isin(['EE','LT','LV']))

LOC = (spark.read.table("common.site_d")
       .filter(site_filter & bu_filter)       
...
# MAGIC %run ./inputDigitalMapsAlternative
...
gdf_loc[gdf_loc['site_bu']=='EE'].plot()
...
# MAGIC %run ./inputSociodemographicsAlternative
...
assert total_POSTCODE_count == EE_POSTCODE_count + LT_POSTCODE_count + LV_POSTCODE_count
...
table_name = 'sociodemographicsAlternative'
print(f'"table_name": {table_name}')
to_databriccks(sd_final, table_name, user_name, project_name)

# COMMAND ----------

table_name = 'sdDictionaryAlternative'
print(f'"table_name": {table_name}')
